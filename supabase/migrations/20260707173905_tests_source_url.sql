-- Step 32 (import provenance UI): adds a nullable tests.source_url column
-- so the app can link to the original forum post ("view original post"),
-- and extends the already-applied ingest_test function (step 31,
-- 20260707150400_ingest_test_function.sql) to accept and store it.
-- See build-history.md step 32 for the full design/rationale.
--
-- This is a new migration layering on top of an already-applied one, not
-- an edit to that file — the same pattern already used for
-- 20260707074426_cascade_delete_clips_and_mapping.sql and others.

alter table public.tests add column source_url text;
...
begin
  ...
  -- Test row.
  insert into public.tests (creator_id, track_id, snapshot_a_id, snapshot_b_id, title, status, source_ref, source_url)
  values (v_owner_id, v_track_id, v_snapshot_a_id, v_snapshot_b_id, v_title, 'open', v_source_ref, v_source_url)
...
-- SECURITY-CRITICAL: re-stated after create-or-replace, since GRANT/REVOKE
-- state is not touched by create-or-replace, but explicit here so this
-- migration is self-contained and the lockdown is visibly re-affirmed.
-- See 20260707150400_ingest_test_function.sql for the original rationale.
