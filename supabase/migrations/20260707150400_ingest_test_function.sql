-- Step 31 (forum ingestion pipeline): atomic ingest of one test's worth of
-- data (track, systems, snapshots, test, clips, clip_mapping, votes).
-- See build-history-ingestion.md step 31 for the full design/rationale.
--
-- Placeholder author resolution (the post author and any voters) happens
-- in application code via lib/ingestion/create-placeholder-author.ts
-- *before* this function is called — auth.admin.createUser() can't run
-- inside a SQL function, so it's necessarily outside this atomic boundary.
-- That's self-healing, not a gap: a retry with the same payload resolves
-- each placeholder to its existing import_authors mapping instead of
-- creating duplicates.
...
begin
  ...
  -- Test row.
  insert into public.tests (creator_id, track_id, snapshot_a_id, snapshot_b_id, title, status, source_ref)
  values (v_owner_id, v_track_id, v_snapshot_a_id, v_snapshot_b_id, v_title, 'open', v_source_ref)
...
-- SECURITY-CRITICAL: this function bypasses RLS entirely (security definer)
-- to write across seven tables as the resolved placeholder owner. Supabase
-- grants EXECUTE on new functions to anon/authenticated by default — left
-- unchanged, any anon-key holder could call this directly via
-- POST /rest/v1/rpc/ingest_test, bypassing every RLS policy in this schema
-- and the ingest route's INGEST_SECRET check entirely. Lock it down to
-- service_role only, the same role the ingest route's admin client uses.
