...
- **Undo path:** `scripts/rollback-lejonklou.ts` / `lib/ingestion/rollback.ts` (step 38, a first version — not this route, it talks to the database directly via the admin client, bypassing this route entirely, same as `ingest_test` itself does). Not yet safe against production once claiming exists — see `build-history-ingestion.md` step 38's notes and `docs/vercel-setup.md`'s rollback-script env var section.
