...
- **Payload validation:** `lib/ingestion/ingest-test-payload.ts` (`validateIngestPayload`) — see that file or `build-history-ingestion.md` step 31 for the full `IngestPayload` shape, including the per-vote `voter` field.
