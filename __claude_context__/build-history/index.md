...
| 85 | Widen `waitForServerState` timeout (8s→15s) and CI e2e retries (1→2) to mitigate real staging replication-lag flakiness in `voting.spec.ts` | [85-e2e-replication-lag-mitigation.md](85-e2e-replication-lag-mitigation.md) |
| 86 | Found the real cause of the `voting.spec.ts` lag (Vercel Data Cache tag-invalidation propagation, not DB replication) via temporary per-attempt logging; re-set the timeout to 30s from measured data | [86-e2e-lag-root-cause-and-retimeout.md](86-e2e-lag-root-cause-and-retimeout.md) |
| 87 | Found the root cause of the Firefox dev-server reload loop (`window.location.reload()` inside Next's own `next-devtools` bundle); no safe app-side fix exists, three remediation attempts documented and rejected | [87-firefox-devtools-reload-root-cause.md](87-firefox-devtools-reload-root-cause.md) |
| 88 | Third color audit — 2 token-alignment fixes (`bg-divider`, `bg-hover-surface` reused for missed call sites) and 6 repeated-class-fragment extractions (4 local consts, new `components/ui/class-names.ts` shared module) | [88-third-color-audit.md](88-third-color-audit.md) |
