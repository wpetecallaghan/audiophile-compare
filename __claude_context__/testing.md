...
## 4. Unit test inventory (56 files · 569 tests · all passing)
...
| `components/ui/__tests__/Link.test.tsx` | 11 | `Link`'s click-interception logic (step 60): a plain click on an internal href calls `preventDefault` + `router.push`; cmd/ctrl/shift/alt-click (`fireEvent`) and middle-click (`userEvent.pointer`) are not intercepted; `target="_blank"` and a non-internal href are not intercepted; a caller-supplied `onClick` still fires and can veto interception via its own `preventDefault()`; not `aria-busy` before any navigation is triggered. Plus (step 63, `useRegisterViewTransition` mocked independently of `document.startViewTransition` support): supported + a resolver available calls `document.startViewTransition` and registers a resolve while still calling `router.push`; falls back to plain navigation when unsupported; falls back when no resolver is available even though supported; the `VIEW_TRANSITION_FALLBACK_MS` safety timer resolves on its own (fake timers) if the registered resolve never fires, and the registered resolve is still safely callable afterward |
| `components/ui/__tests__/ViewTransitionResolver.test.tsx` | 4 | `ViewTransitionResolverProvider`/`useRegisterViewTransition` (step 63, `usePathname`/`useSearchParams` mocked with reassignable values): returns `null` outside a provider; a registered resolve fires exactly once when the pathname changes; does not fire on a rerender with an unchanged pathname and searchParams; fires on a searchParams-only change with the pathname held constant (the `/tests/[id]?from=feed&page=1`-style navigation this app actually needs) |
| `components/ui/__tests__/FooterNavLink.test.tsx` | 2 | `FooterNavLink` (step 68 — the shared wrapper behind the footer's First/Previous/All/Next/Last controls): renders its icon child with the given `href`/`aria-label`; carries the `w-11`/`h-11` touch-target sizing classes (real pixel geometry is checked by an E2E `boundingBox()` assertion instead, since jsdom doesn't do real layout) |
| `lib/nav/__tests__/get-adjacent-ids.test.ts` | 6 | `getAdjacentIds` (step 61 — extracted out of `tests/[id]`'s inline nav formula, now shared by `tests/[id]` and `tracks/[id]`'s First/Previous/All/Next/Last footer nav): an id in the middle of the list resolves all four neighbors; the first id has `prevId`/`firstId` `null`; the last id has `nextId`/`lastId` `null`; a single-item list, a `currentId` not present in `ids`, and an empty `ids` array all resolve to all-`null` |
| `lib/tests/__tests__/format-snapshot-line.test.ts` | 8 | `formatSnapshotLine`: joins both snapshots with their system name; falls back to `'?'` when a snapshot has no joined system; shows only one side when the other is `null`; returns an empty string when both are `null` — visibility gating (`canSeeSystemInfo`, step 43) is the caller's job, not this helper's, see `components.md §8`. Plus (step 65) `formatOneSnapshot` (now exported): formats a single snapshot, falls back to `'?'` with no joined system, returns `null` for a `null` snapshot |
...
| Spec | Scenarios |
|---|---|
| `public-feed.spec.ts` | Feed loads; unauthenticated header; test card structure; `/systems` → login redirect with redirectTo; `/about` loads without a redirect; `/profile` → login redirect; `/tracks` → login redirect; anonymous visitor can play clips on a test detail page and sees a "Sign in to vote" prompt instead of the vote form; `/register` shows the Google sign-up option alongside the email form; on a throttled connection, navigating between feed pages shows the `<PageLoading>` skeleton, lands on the right page, and Privacy/Terms — polled every 100ms throughout the whole transition, not just checked once — never becomes visible on a narrow viewport (step 66, extended step 74); feed pagination controls have a real `boundingBox()` of at least 44×44px (step 68); on a throttled connection, a test detail page's footer nav (`TestNavFooter`) and clip player (`ClipPlayerSection`) both still stream in and resolve — confirms the two independent `Suspense` boundaries from step 69 don't hang or error under a slow connection; footer height stays constant across feed pagination at a 390px viewport (step 74) — regression coverage for `SiteFooter`'s wrap-to-two-lines bug; Privacy/Terms links are hidden on mobile only on routes that show footer nav (step 74), including with JavaScript disabled entirely — proves the decision (`FooterPrivacyLinks.tsx`'s `usePathname()` match) is resolved server-side, not dependent on `FooterPortal`'s client-side mount timing |
...
| `voting.spec.ts` | Tally hidden before voting; vote count visible; system/snapshot info visible to the test's creator before reveal but hidden from a non-creator (`canSeeSystemInfo`, step 43); cast vote (Tune Method only, step 57); update existing vote; creator can reveal; system/snapshot info visible to a non-creator too once revealed — now sourced from `MappingBadge`, not the header line, which is hidden once revealed (step 65); a creator-added forum link is hidden from a non-creator until reveal and stays editable after reveal and after a vote exists (step 46). `revealedStatus` exact-text assertions no longer need `.first()` (step 67 — `MappingBadge` no longer renders its own duplicate "Revealed" text). Footer step-through nav (step 69): Next/Previous are exact inverses and First/Last land at the true ends of the feed's ordering regardless of the fixture test's absolute position — the first real assertion coverage for First/Previous/All/Next/Last, previously untested since step 61 introduced it. Header spoofing is rejected (step 71): an anonymous viewer forging `x-user-id` to the real creator's id via `page.setExtraHTTPHeaders` still doesn't see the creator-only Reveal control, proving `middleware.ts`'s unconditional header strip holds |
