...
## 4. Unit test inventory (55 files · 567 tests · all passing)
...
| `components/tests/__tests__/MappingBadge.test.tsx` | 5 | Before/After labeling per `clipAId === beforeClipId` (both directions, step 65); each side's snapshot summary (`snapshotA`/`snapshotB`) renders next to its label; summary line omitted when a snapshot is `null`; the unsupported-clip link's own text stays just "Before"/"After", not the snapshot summary rendered alongside it |
